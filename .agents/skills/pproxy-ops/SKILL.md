...
description: |
  ...
  - 商业多租户配额："生成密钥对"、"签发用户令牌"、"撤销令牌"、"区分普通用户与管理员" (pproxy user keygen/add/revoke)
  - 桌面客户端双模大盘："普通租户单链路视图"、"管理员全局集群与三出口监控" (Dashboard 身份感知)
...
### 3.2 离线签发多租户令牌
```bash
# 签发 50GB 流量、30 天有效、最大 3 并发的个人普通租户令牌
pproxy user add alice -q 50G -d 30 -c 3

# 短参数极速模式
pproxy user add bob -q 20G -d 15 -c 2

# 签发管理员令牌（role: admin，登录桌面端开启全局多出口监控与集群大盘）
pproxy user add admin -q 500G -d 365 -c 10
```
- **普通用户令牌**：在桌面端输入生成的 `usr_live_...` 令牌后，界面呈现极简模式：隐藏底层 3 个复杂物理出口，仅显示单一【代理状态（端到端链路）】、常用站点测速，以及专属的个人配额进度条（已用流量、总配额、到期时间、并发上限）；
- **管理员令牌**：若用户名以 `admin` 命名或携带 `role: admin`，桌面端自动切换至【管理员全局视图】：展示出口R / 出口C / 出口V 完整独立监控，以及分布式多节点集群状态大盘（实测各节点在线状态与内网 RTT 延迟）。
