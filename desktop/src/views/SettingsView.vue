...
// ---- auto_proxy 开关（Fix4 T5） ----
const autoProxy = ref<boolean>(false)
async function refreshAutoProxy(): Promise<void> {
  try { const cfg = await loadAutoProxyConfig(); autoProxy.value = Boolean(cfg.auto_proxy) } catch { autoProxy.value = false }
}
async function toggleAutoProxy(v: boolean): Promise<void> {
  autoProxy.value = v
  try { await saveAutoProxyConfig({ auto_proxy: v }) } catch {}
}
...
<template>
  <div class="max-w-xl space-y-6">
    ...
    <Card>
      <CardHeader>
        <CardTitle class="text-sm">系统代理</CardTitle>
        <CardDescription>自动开启系统代理以修复连接问题（可在托盘中随时开关）。</CardDescription>
      </CardHeader>
      <CardContent class="flex items-center gap-3">
        <label class="flex items-center gap-2 text-sm">
          <input type="checkbox" :checked="autoProxy" @change="toggleAutoProxy(($event.target as HTMLInputElement).checked)" class="rounded" />
          自动开启系统代理
        </label>
      </CardContent>
    </Card>
