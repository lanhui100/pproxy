...
**状态**：`阶段 3 — dev 部署与实测`（Pending，需 dev 服务器操作）
**当前断点**：阶段 2 完成（53/53 pproxy-cli，9fa17c2）；T3 需在 dev 服务器上 cargo build + restart + AC1-5 验收。
**Next Action**：T3 —— dev 上 `cargo build --release`、`systemctl restart pproxy`、执行 §6.2 手工清单（AC1–AC5）。
**Resume Hint**：新会话恢复时，读本节 + §4 阶段表；实测细节以 §6.2 为准；dev env 已就绪（§0 预核查）。
...
| 阶段 2 | `pproxy doctor` 增加 CONNECT 隧道探针 | **Done** | `crates/cli/src/cmd/doctor.rs`；classify_probe_result 纯函数 + 8 条单测（9fa17c2） |
